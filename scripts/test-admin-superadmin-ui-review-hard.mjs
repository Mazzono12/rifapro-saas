...
for (const token of [
  "Dashboard global", "Financeiro global", "Auditoria global",
  "Integrações globais", "Domínios", "Antifraude"
]) {
...
for (const token of [
  ...
  assert.ok(!superadmin.includes(`name: "${token}"`), `Menu Superadmin duplicado deveria sair: ${token}`);
}
...
assert.ok(superadminDashboard.includes("Entrar no ambiente"), "Gestão de tenants precisa manter botão para entrar no ambiente do inquilino.");
...
assert.ok(superadminDashboard.includes('to={`/superadmin/tenants/${raffle.tenant_id}/financeiro`}'), "Listagem global de rifas precisa manter atalho para o tenant da rifa.");

mkdirSync("reports/hard", { recursive: true });
writeFileSync("reports/hard/admin-superadmin-ui-review-hard.json", JSON.stringify({
  ok: true,
  checked: ["admin_menu", "superadmin_menu", "routes", "responsive_shell", "accessibility", "tenant_environment_access", "global_raffle_actions"]
}, null, 2));
