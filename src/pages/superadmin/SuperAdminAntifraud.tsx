  ...
  return (
    <div className="space-y-6">
      ...
      <AdminDataTable
        ...
        rows={cases.map((item: any) => [item.tenant || item.client || "Cliente", item.signal_type, item.score, item.severity, item.action, item.status])}
        empty="Nenhum caso antifraude global."
      />
      <AdminDataTable
        columns={["Cliente", "Evento", "Score", "Severidade", "Status", "Observação"]}
        rows={signals.slice(0, 40).map((item: any) => [item.tenant || item.client || "Cliente", item.signal_type, item.score || 0, item.severity, item.status, "Dados protegidos"])}
