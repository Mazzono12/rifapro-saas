...
export function Signup() {
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Cadastro criado", { description: "Agora faca login para iniciar a sessao." });
  ...
  return (
    <AuthShell title="Criar conta SaaS" subtitle="Cadastro inicial para tenant dev. O backend associa o tenant de forma confiavel.">
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <label className="block">
          ...
          <span className="relative block">
            ...
            <input value={password} onChange={event => setPassword(event.target.value)} type="password" minLength={6} required className="w-full rounded-2xl border border-white/10 bg-black/30 py-4 pl-12 pr-4 text-white outline-none transition focus:border-cyan-300/60" placeholder="Minimo 6 caracteres" />
          </span>
        </label>
