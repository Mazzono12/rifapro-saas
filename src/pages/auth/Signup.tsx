      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <label className="block">
          ...
          <span className="relative block">
            ...
            <input value={password} onChange={event => setPassword(event.target.value)} type="password" minLength={6} required className="w-full rounded-2xl border border-white/10 bg-black/30 py-4 pl-12 pr-4 text-white outline-none transition focus:border-cyan-300/60" placeholder="Mínimo de 6 caracteres" />
          </span>
        </label>
        ...
        <p className="text-center text-sm text-slate-400">Já tem acesso? <Link to="/login" className="text-cyan-200">Entrar</Link></p>
      </form>
    </AuthShell>
  );
}
