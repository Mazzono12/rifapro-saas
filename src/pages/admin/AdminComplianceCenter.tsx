...
export function AdminComplianceCenter({ view = "compliance" }: { view?: ComplianceView }) {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Metric icon={WalletCards} label="Lancamentos financeiros" value={wallet.length} />
        </div>
  ...
  if (view === "antifraud") {
    return (
      <div className="space-y-6">
        ...
        <Section title="Eventos de score" rows={fraud.map(item => [item.signal_type, `${item.severity} (${item.score ?? 0})`, item.status, "Dados protegidos"])} />
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Metric icon={FileSearch} label="Eventos auditaveis" value={audit.length} />
        </div>
        <Section title="Gerenciar cotas do pedido" rows={adjustments.map(item => [item.adjustment_type, item.order_id, item.old_numbers?.join(", "), item.new_numbers?.join(", ")])} />
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Metric icon={ShieldAlert} label="Solicitacoes LGPD" value={compliance?.requests?.length || 0} />
      </div>
...
function Section({ title, rows }: { title: string; rows: ReactNode[][] }) {
  return (
    <section className="admin-card overflow-hidden p-0">
      <div className="border-b border-[var(--admin-border)] p-4">
