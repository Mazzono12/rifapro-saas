...
CREATE POLICY "Admins manage caixinha config" ON public.caixinha_configuracoes FOR ALL USING (auth.is_admin());
CREATE POLICY "Admins manage caixinha prizes" ON public.caixinha_premios FOR ALL USING (auth.is_admin());
CREATE POLICY "Users view own hidden boxes" ON public.caixinhas FOR SELECT USING (usuario_id = auth.uid() OR auth.is_admin());
CREATE POLICY "Admins manage caixinhas" ON public.caixinhas FOR ALL USING (auth.is_admin());
CREATE POLICY "Admins manage caixinha rewards" ON public.caixinha_recompensas FOR ALL USING (auth.is_admin());
CREATE POLICY "Users view own caixinha history" ON public.caixinha_historico FOR SELECT USING (usuario_id = auth.uid() OR auth.is_admin());
CREATE POLICY "Admins manage caixinha history" ON public.caixinha_historico FOR ALL USING (auth.is_admin());
